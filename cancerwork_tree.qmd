...
boxm <- boxM(filtered_data[,-1], filtered_data$class)
print(boxm)

shapiro.test(filtered_data[,2])  
shapiro.test(filtered_data[,3])
...
```{r}
class_lda <- lda(class~., data = filtered_data)
class_pred_lda <- predict(class_lda, filtered_test)

acc_lda <- mean(class_pred_lda$class == filtered_test$class)
print(acc_lda)
table(class_pred_lda$class, filtered_test$class)
...
```{r}
class_qda <- qda(class~., data = filtered_data)

class_pred_qda <- predict(class_qda, filtered_test)

acc_qda <- mean(class_pred_qda$class == filtered_test$class)
print(acc_qda)
table(class_pred_qda$class, filtered_test$class)
...
#| fig-cap: ROC 
roc_lda <- roc(filtered_test$class, class_pred_lda$posterior[,2])
plot(roc_lda, main = "ROC for LDA", print.auc = T, auc.polygon = T, legacy.axes = T)

roc_qda <- roc(filtered_test$class, class_pred_qda$posterior[,2])
