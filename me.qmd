...
A classification tree is typically suitable for smaller datasets because it is easy to interpret during the training process and can quickly generate predictions.
...
The final prediction is determined by a majority vote, where the most frequent class across all predictions is chosen.
To find the minimum number of trees that stabilize the OOB error, the model was trained with varying tree numbers, and the OOB error was monitored.
...
According to the Bagging Tree, Variable.2640 is the most important factor, followed by Variable.4192 and Variable.6481, Variable.132 and Variable.1193, Variable.6442and Variable.1235 are relatively unimportant.The Accuracy of test is 0.79, which is good.
...
Similarly, the model was trained using the dataset after initial variable selection.
...
According to the random forests,the Variable.6481 is the most important factor, Variable.2640 and Variable.4192, which were also important in the Bagging model, remain influential.
...
Reducing the features from 5000 to 1824 improved accuracy and AUC after tuning.
PCA (47 features) maintained good accuracy but slightly underperformed compared to feature selection.
GridSearchCV generally improved AUC.
